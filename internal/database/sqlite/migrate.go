...
func updateVersionInMetadataTable(ctx context.Context, tx *sql.Tx, newVersion int) error {
	query := `
    INSERT INTO e_migration (applied_at, version)
    VALUES (?, ?)
    `
	_, err := tx.ExecContext(ctx, query, time.Now().UTC(), newVersion)
	return err
}
...
func getCurrentVersion(ctx context.Context, tx *sql.Tx) (int, error) {
	...
	if err != nil {
		...
		if strings.Contains(strings.ToLower(err.Error()), "no such table: e_migration") {
